...
def build_mouth_roi_output_path(video_path: str, output_root: str = "data/mouth_roi") -> str:
    ...
    if parent in {"mouth_roi", "mouth_roi_with_audio"}:
        out_name = f"{name}.mp4"
    elif parent and not name.startswith(f"{parent}_"):
...
def resolve_inference_video_path(video_path: str, cfg: DictConfig, modality: str) -> str:
    ...
    if not use_mouth_roi_input:
        return video_path

    mouth_roi_root = cfg.get("mouth_roi_root", "data/mouth_roi")
    fallback_roots = cfg.get("mouth_roi_fallback_roots", ["data/mouth_roi_with_audio", "data/mouth_roi"])
    if isinstance(fallback_roots, str):
        fallback_roots = [fallback_roots]

    roi_candidates = []

    normalized_input = os.path.normpath(video_path)
    input_parts = normalized_input.split(os.sep)
    if any(part in {"mouth_roi", "mouth_roi_with_audio"} for part in input_parts):
        roi_candidates.append(video_path)

    for root in [mouth_roi_root, *fallback_roots]:
        if not root:
            continue
        roi_candidate = build_mouth_roi_output_path(video_path, output_root=root)
        if roi_candidate not in roi_candidates:
            roi_candidates.append(roi_candidate)

    for roi_path in roi_candidates:
        if os.path.exists(roi_path):
            log.info("Using mouth ROI input for modality=%s: %s", modality_key, roi_path)
            return roi_path

    if roi_candidates:
        log.warning("Mouth ROI not found. Tried paths: %s", ", ".join(roi_candidates))
...
def prepare_inference_components(
    cfg: DictConfig,
    device: torch.device,
    model: Optional[E2E] = None,
    beam_search: Optional[BatchBeamSearch] = None,
) -> Tuple[E2E, BatchBeamSearch]:
    """Prepare reusable model + beam search components.

    If `model` and `beam_search` are provided, they are reused as-is.
    Otherwise, missing components are created from config.
    """
    if model is None:
...
def transcribe(
    ...
    if modality_key in ("v", "av") and input_ext in AUDIO_EXTS:
        raise ValueError(
            f"Modality '{modality_key}' requires video input, but got audio file: {video_path}. "
            "Use modality='a' for audio files."
        )
    ...
    if modality_key == "a":
        if input_ext in AUDIO_EXTS:
            ...
            log.info(
                "Audio modality received non-audio extension '%s'; "
                "trying to decode container/sidecar audio from %s",
                input_ext,
                video_path,
            )
    ...
    # --- model ---------------------------------------------------------------
    model, beam_search = prepare_inference_components(
        cfg,
        device,
        model=model,
...
def evaluate_from_csv(cfg: DictConfig, device: torch.device, modality: str, detector: str):
    ...
    model, beam_search = prepare_inference_components(cfg, device)
