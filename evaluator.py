...
class USREvaluator(LightningModule):
    ...
    def test_step(self, data, batch_idx, dataloader_idx=0):
        # Some samples can fail during video loading; collate_fn then yields None for a whole empty batch.
        if data.get("video") is None or data.get("audio") is None or data.get("label") is None:
            self.skipped_test_batches += 1
            if self.skipped_test_batches <= 5:
                self.print(
                    f"Skipping empty/invalid test batch at index {batch_idx}. "
                    "This usually means media decoding failed for all samples in that batch."
                )
            return
    ...
    def on_test_epoch_end(self):
        if self.skipped_test_batches > 0:
            self.print(f"Skipped {self.skipped_test_batches} invalid test batches.")
