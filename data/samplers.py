...
def batch_by_size(indices, num_tokens_fn, max_tokens):
    """Group sorted indices into batches where total tokens per batch <= max_tokens.

    Pure-Python replacement for fairseq's Cython ``batch_by_size``.

    Args:
        indices: 1-D array of dataset indices (typically sorted by length, descending).
        num_tokens_fn: callable that returns the number of tokens for a given index.
        max_tokens: maximum total tokens in a single batch.

    Returns:
        List of lists, where each inner list is a batch of indices.
    """
    batches = []
    batch = []
    cur_max = 0
    ...
    for idx in indices:
        ntok = num_tokens_fn(idx)
        # Would adding this sample exceed max_tokens?
        new_max = max(cur_max, ntok)
        if batch and new_max * (len(batch) + 1) > max_tokens:
            batches.append(batch)
            batch = []
            cur_max = 0
        batch.append(idx)
        cur_max = max(cur_max, ntok)
...
class ByFrameCountSampler(Sampler):
    def __init__(self, dataset, max_frames_per_gpu, shuffle=True, seed=0):
        ...
        self.sizes = [item[2] for item in self.dataset.samples]
